...
import com.osp.biz.service.consumer.OspContentService;
import com.osp.biz.service.consumer.OspPaybindService;
...
public class DemoAction {
	...
	public void setOspContentService(OspContentService ospContentService) {
		this.ospContentService = ospContentService;
	}

	private OspPaybindService ospPaybindService;

	public void setOspPaybindService(OspPaybindService ospPaybindService) {
		this.ospPaybindService = ospPaybindService;
	}

	public void start() throws Exception {

		OspPaybind ospPaybind =new OspPaybind();
		ospPaybind.setBalance(Float.valueOf(1234));
		ospPaybind.setBindflag("1");
		ospPaybind.setDefaultflag("0");
		ospPaybind.setUsername("wfw1");
		ospPaybind.setPayaccount("324234");
		ospPaybindService.save(ospPaybind);
		
		for (int i = 0; i < 50; i++) {
			OspContent t = ospContentService.getById(new BigDecimal(
					"201112092151590093").longValue());
			System.out.println("["
			...
			System.out.println("["
					+ new SimpleDateFormat("HH:mm:ss").format(new Date())
					+ "] " + ospContentService.findByDynamicWhere(t).size());
			Thread.sleep(2000);
		}
		...
		System.out.println("SUCCESS: got bid response id: "
				+ bidService.bid(request).getId());
		// }
		...
		System.out.println("SUCCESS: registered user with id "
				+ anotherUserRestService.registerUser(user).getId());

		RpcContext.getContext().setAttachment("clientName", "demo");
		RpcContext.getContext().setAttachment("clientImpl", "dubbox");
		System.out.println("SUCCESS: got user "
				+ anotherUserRestService.getUser(1L));
