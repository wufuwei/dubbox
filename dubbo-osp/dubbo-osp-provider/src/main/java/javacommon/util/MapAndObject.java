...
/**
 * 混合两个对象为一个Map对象 MapAndObject.get()方法将在两个对象取值,Map如果取值为null,则再在Bean中取值
 * 
 * @author badqiu
 */
...
public class MapAndObject implements Map {
	...
	private static class FastPropertyUtils {
		private static Object getBeanPropertyValue(Object bean,
				String propertyName) {
			if (bean == null)
				throw new IllegalArgumentException("bean cannot be not null");
			if (propertyName == null)
				throw new IllegalArgumentException(
						"propertyName cannot be not null");
			...
			try {
				...
				throw new IllegalStateException(
						"cannot get property value by property:" + propertyName
